...
from research.learning_unsupervised_learning import variable_replace
    ...
    def _build_once(self, dataset, feature_transformer):
        with tf.device(self._local_device):
            ...
            if self.hinge_loss:
                ...
                mse_test = tf.reduce_sum(
                    tf.nn.relu(tf.reduce_sum(tf.square(logit_test - y_test), axis=1) - 1.)) / batch_size
            else:
            ...
            def test_confusion_fn():
                ...
                test_confusion = tf.to_float(test_confusion) / tf.constant(
                    (logit_test.shape.as_list()[0] / float(logit_test.shape.as_list()[1])), dtype=tf.float32)
            ...
            loss_to_train_at = (reg + mse_test) * self.test_train_scalar + (mse_train + reg) * (
                        1 - self.test_train_scalar)
