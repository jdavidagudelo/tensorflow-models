...
from research.differential_privacy.multiple_teachers import aggregation
from research.differential_privacy.multiple_teachers import deep_cnn
from research.differential_privacy.multiple_teachers import input
from research.differential_privacy.multiple_teachers import metrics
    ...
    for teacher_id in xrange(nb_teachers):
        ...
        if FLAGS.deeper:
            ckpt_path = FLAGS.teachers_dir + '/' + str(dataset) + '_' + str(nb_teachers) + '_teachers_' + str(
                teacher_id) + '_deep.ckpt-' + str(FLAGS.teachers_max_steps - 1)  # NOLINT(long-line)
        else:
            ckpt_path = FLAGS.teachers_dir + '/' + str(dataset) + '_' + str(nb_teachers) + '_teachers_' + str(
                teacher_id) + '.ckpt-' + str(FLAGS.teachers_max_steps - 1)  # NOLINT(long-line)
...
def prepare_student_data(dataset, nb_teachers, save=False):
  ...
  """
    ...
    if not save:
        ...
        stdnt_labels, clean_votes, labels_for_dump = aggregation.noisy_max(teachers_preds, FLAGS.lap_scale,
    ...
    if save:
        ...
        filepath = FLAGS.data_dir + "/" + str(dataset) + '_' + str(nb_teachers) + '_student_labels_lap_' + str(
            FLAGS.lap_scale) + '.npy'  # NOLINT(long-line)
