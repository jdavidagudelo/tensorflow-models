...
from research.gan.cyclegan import inference_demo
from research.gan.cyclegan import train
...
class InferenceDemoTest(tf.test.TestCase):
    ...
    def setUp(self):
        self._export_dir = os.path.join('/tmp', 'export')
        self._ckpt_path = os.path.join(self._export_dir, 'model.ckpt')
        self._image_glob = os.path.join(
            '',
            'testdata', '*.jpg')
        self._genx_dir = os.path.join('', 'genx')
        self._geny_dir = os.path.join('', 'geny')
