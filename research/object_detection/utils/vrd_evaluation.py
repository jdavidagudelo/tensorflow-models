    ...
    """
        ...
        if relationships:
            for key in average_precisions:
                average_precision = average_precisions.get(key)
...
class VRDPhraseDetectionEvaluator(VRDDetectionEvaluator):
  ...
  """
    ...
    """
        first_box_key = [k for k in groundtruth_box_tuples.dtype.fields][0]
    ...
    Returns:
      result: A numpy array of structures with the shape [M, 1], each
        ...
        of all bounding boxes of the i-th input structure.
    """
        first_box_key = [k for k in detections_box_tuples.dtype.fields][0]
...
class _VRDDetectionEvaluation(object):
  ...
  """
    ...
    """
        ...
        for relation_field_value in self._num_gt_instances_per_relationship:
        ...
        self._mean_average_precision = np.mean(
            [self._average_precisions.get(key) for key in self._average_precisions])
