...
class CocoDetectionEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    Returns:
      ...
      'DetectionBoxes_Precision/mAP': mean average precision over classes
        averaged over IOU thresholds ranging from research.object_detection5 to .95 with .05
        increments.
...
class CocoMaskEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    Returns:
      ...
      'DetectionMasks_Precision/mAP': mean average precision over classes
        averaged over IOU thresholds ranging from research.object_detection5 to .95 with .05 increments.
    ...
    """
        groundtruth_dict = {
            'annotations': self._groundtruth_list,
            'images': [{'id': image_id,
                        'height': self._image_id_to_mask_shape_map.get(image_id)[1],
                        'width': self._image_id_to_mask_shape_map.get(image_id)[2]
                        }
                       for image_id in self._image_id_to_mask_shape_map],
            'categories': self._categories
        }
        ...
        mask_metrics = {'DetectionMasks_' + key: mask_metrics.get(key)
