...
class TfExampleDecoderTest(tf.test.TestCase):
    ...
    def testDecodeJpegImage(self):
        ...
        self.assertAllEqual([4, 5], tensor_dict[fields.InputDataFields.
                            original_image_spatial_shape])
        self.assertEqual(b'image_id', tensor_dict[fields.InputDataFields.source_id])
    ...
    def testDecodeImageKeyAndFilename(self):
        ...
        self.assertEqual(b'filename', tensor_dict[fields.InputDataFields.filename])
    ...
    def testDecodeObjectLabelUnrecognizedName(self):
        image_tensor = np.random.randint(256, size=(4, 5, 3)).astype(np.uint8)
        encoded_jpeg = self._EncodeImage(image_tensor)
        bbox_classes_text = [b'cat', b'cheetah']
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    'image/encoded':
