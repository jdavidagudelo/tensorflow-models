...
from research.steve import valuerl
  ...
  """
    ...
    def __init__(self, proc_num, evaluation, policy_lock, batch_size, config):
        ...
        self.log_path = util.create_directory("%s/%s/%s/%s" % (
            config["output_root"], config["env"]["name"], config["name"], config["log_path"])) + "/%s" % config["name"]
        self.load_path = util.create_directory(
            "%s/%s/%s/%s" % (config["output_root"], config["env"]["name"], config["name"], config["save_model_path"]))
        ...
        self.policy_actions = self.valuerl.build_evalution_graph(self.obs_loader,
                                                                 mode="exploit" if self.evaluation else "explore")

        # interactors
        self.agent_pipes, self.agent_child_pipes = list(zip(*[multiprocessing.Pipe() for _ in range(self.batch_size)]))
        self.agents = [multiprocessing.Process(target=run_env, args=(self.agent_child_pipes[i],)) for i in
        ...
        for i, (t, r, reset) in enumerate(zip(self.total_rewards, rewards, resets)):
            if reset:
                ...
                if self.evaluation and self.loaded_policy:
                    with portalocker.Lock(self.log_path + '.greedy.csv', mode="a") as f: f.write(
    ...
    def reload(self):
        ...
        with self.policy_lock:
            ...
            self.epoch, self.frame_total, self.hours = self.sess.run(
                [self.valuerl.epoch_n, self.valuerl.frame_n, self.valuerl.hours])
...
def main(proc_num, evaluation, policy_replay_frame_queue, model_replay_frame_queue, policy_lock, config):
    try:
        ...
        agentmanager = AgentManager(proc_num, evaluation, policy_lock,
                                    config["evaluator_config"]["batch_size"] if evaluation else config["agent_config"][
                                        "batch_size"], config)
        frame_i = 0
        while True:
