...
from research.fivo.experimental import bounds
from research.fivo.experimental import data
from research.fivo.experimental import models
from research.fivo.experimental import summary_utils as summ
...
def create_graph(bound, state_size, num_timesteps, batch_size,
    ...
    if FLAGS.use_bs:
        ...
        true_bs = [np.zeros([state_size]).astype(dtype) for _ in range(num_timesteps)]
...
def parse_resampling_schedule(schedule, num_timesteps):
    ...
    if schedule == "always":
        ...
        return [(i + 1) % n == 0 for i in range(num_timesteps)]
    else:
