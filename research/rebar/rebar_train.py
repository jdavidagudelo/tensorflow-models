...
from research.rebar import rebar
from research.rebar import datasets
from research.rebar import logger as L
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop():
            ...
            if isinstance(grad_variances, list):
                ...
                grad_variances = '\n'.join(map(str, sorted(
                  (k, grad_variances.get(k)) for k in grad_variances)))
            else:
                rowkey['step'] = step
                logger.log(rowkey, {'step': step,
                                    'train': np.mean(lHats, axis=0)[0],
