...
from research.rebar import utils as U
...
class SBN(object):  # REINFORCE
    ...
    '''
        ...
        if self.hparams.task in ['sbn', 'omni']:
            ...
            with slim.arg_scope([slim.fully_connected],
                ...
                for i in reversed(range(self.hparams.n_layer)):
                    if i == 0:
    ...
    """
        ...
        muELBOGrads = tf.gradients(tf.reduce_sum(muELBO),
                                   [muSamples[i]['activation'] for
                                    i in range(self.hparams.n_layer)])
        ...
        for i in range(self.hparams.n_layer):
            ...
            learning_signals.append(
                learning_signal - scaling_baseline_0 * muELBO - scaling_baseline_1 * dfDiff - self._create_baseline())
    ...
    '''
        ...
        h = 0
        extra = []
        for layer in range(self.hparams.n_layer):
            logQ, softSamples = self._recognition_network(sampler=functools.partial(
    ...
    def get_dynamic_rebar_gradient(self):
        ...
        reparam_g_t = \
        tf.gradients(tf.reduce_mean(2 * tf.stop_gradient(g - gbar) * reparam_g), self.pre_temperature_variable)[0]
