  ...
  """
    ...
    with tf.python_io.TFRecordWriter(tmp_path) as writer:
        for i in range(num_examples):
            ...
            features_dict = {'sequence': _as_bytes_feature((char * read_len).encode())}
            for target_name in TEST_TARGETS:
                nonsense_label = _as_bytes_feature(str(i).encode())
  ...
  """
    ...
    for target in TEST_TARGETS:
        dataset_info.labels.add(
            name=target, values=[str(i) for i in range(num_examples)])

    tmp_path = os.path.join('/tmp', 'test.pbtxt')
